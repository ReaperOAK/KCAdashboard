...
const ChessHome = React.memo(function ChessHome() {
  ...
  return (
    <section
      className="flex flex-col items-center justify-center min-h-[40vh] h-[60vh] text-center bg-background-light px-4"
      aria-live="polite"
      aria-label="Chess dashboard redirect"
      role="status"
    >
      <h1 className="text-2xl sm:text-3xl font-bold text-primary mb-4">Chess Dashboard</h1>
      <p className="text-base sm:text-lg text-gray-dark">Redirecting to chess player interface...</p>
    </section>
  );
});
